...
import { fetchAcademicPayment, getMyAcademicPayments } from "../../../slice/admin/paymentSlice";
// Dropdwon
function Sujets() {
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      Cell: (row) => {
        // console.log(row)
        return <span>{row?.cell?.value.lastname +" "+ row?.cell?.value.name}</span>;
      },
    },
    {
      Header: "Montant",
  ...
  useEffect(() => {
    dispatch(fetchAcademicPayment());
  }, []);

 
  const {academicPayment:payment,loading:loadingPayment}  = useSelector((state) => state.paymentSlice);
console.log(payment)
  ...
  return (
    <div className="space-y-5">
      <HomeHeader title="Paiements"/>
       

      {loadingPayment == false && (
        <Datatables title="Liste des paiements" content={payment} col={COLUMNS} />
      )}
    </div>
  );
}
